...
const AUG={"Level Up!":["6MaxLevel103","Prismatic"],"Pandora's Items III":["9PandorasRadiantBox3","Prismatic"],"Buried Treasures":["9BuildingACollectionPlusPlus3","Prismatic"],"Living Forge":["7LivingForge3","Prismatic"],"Glass Cannon II":["GlassCannonII2","Gold"],"Cybernetic Uplink":["6CyberneticUplink22","Gold"],"Cybernetic Implants":["6CyberneticImplants22","Gold"],"Sunfire Board":["6SunfireBoard2","Gold"],"Ascension":["9Commander_Ascension2","Gold"],"Trade Sector":["6TradeSector2","Gold"],"Big Grab Bag":["9BigGrabBag2","Gold"],"Portable Forge":["6PortableForge2","Gold"],"Second Wind II":["6SecondWind22","Gold"],"Glass Cannon I":["GlassCannonI1","Silver"],"Carve a Path":["ComponentQuestSword1","Silver"],"Continuous Conjuration":["ComponentQuestRod1","Silver"],"Backup Bows":["ComponentQuestBow1","Silver"],"Extra Buckles":["ComponentQuestBelt1","Silver"],"Tiny Titans":["6TinyTitans1","Silver"],"Pandora's Items":["6PandorasItems1","Silver"],"Second Wind":["6SecondWind11","Silver"],"Recombobulator":["6Recombobulator1","Silver"]};
...
const A_AP=/Rabadon|Void Staff|Jeweled|Blue Buff|Nashor|Hextech|Morellonomicon|Archangel|Ionic|Crownguard/,A_ADM=/Titan's|Sterak|Bloodthirster/,A_ADR=/Infinity Edge|Deathblade|Last Whisper|Giant Slayer|Kraken|Guinsoo|Red Buff/,A_ASP=/Kraken|Guinsoo|Red Buff|Nashor/;
function pickAug(carries,roll,lvl){const items=carries.flatMap(x=>x.items.map(i=>i.name)).join(' ');const main=(carries[0]?carries[0].items:[]).map(i=>i.name).join(' ');const dmg=A_AP.test(main)?'ap':A_ADM.test(main)?'adm':A_ADR.test(main)?'adr':A_AP.test(items)?'ap':A_ADM.test(items)?'adm':A_ADR.test(items)?'adr':'tank';const fast=roll==='fast',reroll=roll==='slow'&&lvl<=6;const w=[];carries.map(x=>x.name).filter(n=>HERO[n]).slice(0,2).forEach(n=>w.push(mkH(n)));w.push(mkG(fast?"Level Up!":reroll?"Buried Treasures":"Pandora's Items III"));w.push(mkG(dmg==='ap'?"Cybernetic Uplink":dmg==='adm'?"Cybernetic Implants":dmg==='adr'?"Glass Cannon II":"Sunfire Board"));w.push(mkG(fast?"Ascension":"Trade Sector"));w.push(mkG(reroll?"Second Wind":dmg==='ap'?"Continuous Conjuration":dmg==='tank'?"Extra Buckles":(dmg==='adr'&&A_ASP.test(items))?"Backup Bows":"Carve a Path"));w.push(mkG(reroll?"Tiny Titans":"Pandora's Items"));w.push(mkG("Glass Cannon I"),mkG("Recombobulator"),mkG("Tiny Titans"));const seen=new Set(),out=[];for(const a of w){if(out.length>=5)break;if(a&&!seen.has(a.n)){seen.add(a.n);out.push(a);}}return out;}
